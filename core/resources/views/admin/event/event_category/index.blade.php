...
@section('content')
  <div class="page-header">
    <h4 class="page-title">Event Manage Categories</h4>
                    ...
                    <tbody>
                      @foreach ($event_categories as $key => $event_category)
                        <tr>
                          ...
                          <td>
                            <a class="btn btn-secondary btn-sm editbtn" href="#editModal" data-toggle="modal" data-bcategory_id="{{$event_category->id}}" data-name="{{$event_category->name}}" data-status="{{$event_category->status}}">
                              <span class="btn-label">
                                <i class="fas fa-edit"></i>
                              </span>
                              Edit
                            </a>
                            <form class="deleteform d-inline-block" action="{{route('admin.event.category.delete')}}" method="post">
                              @csrf
        ...
        <div class="card-footer">
          <div class="row">
            <div class="d-inline-block mx-auto">
              {{$event_categories->appends(['language' => request()->input('language')])->links()}}
            </div>
          </div>
        </div>
      </div>
  ...
  <div class="modal fade" id="editModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered" role="document">
      <div class="modal-content">
        <div class="modal-header">
          <h5 class="modal-title" id="exampleModalLongTitle">Edit Event Category</h5>
          <button type="button" class="close" data-dismiss="modal" aria-label="Close">
            <span aria-hidden="true">&times;</span>
          </button>
        </div>
        <div class="modal-body">
          <form id="ajaxEditForm" class="" action="{{route('admin.event.category.update')}}" method="POST">
            @csrf
            <input id="inbcategory_id" type="hidden" name="event_category_id" value="">
            <div class="form-group">
              <label for="">Name **</label>
